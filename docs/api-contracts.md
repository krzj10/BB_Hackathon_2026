...
**Discriminated-union boundaries (cross-language rule):** member models keep their canonical literal defaults (`kind`/`tool`/`type`), but every union boundary makes the discriminator mandatory - in JSON Schema via an added `required` on the boundary entry, and in TypeScript via `RequireDiscriminator<Member, "kind">`. An inline discriminated union anywhere (e.g. `Meeting.span`) resolves to the same strengthened alias. The generated `EventEnvelope` is an explicit correlated union (`EventEnvelopeBase & { type: <literal>; payload: <Member> }` per catalog entry), so a mismatched outer `type`/payload is a TypeScript compile error and narrowing by `event.type` types `event.payload` correctly; the Pydantic validator remains as runtime defense in depth. JSON Schema intentionally does not encode outer/inner correlation (documented; enforced by Pydantic + TS).
